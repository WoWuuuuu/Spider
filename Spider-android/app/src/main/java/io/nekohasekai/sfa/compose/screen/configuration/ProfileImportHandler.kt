...
class ProfileImportHandler(private val context: Context) {
        ...
        try {
            ...
            val content =
                try {
                    ...
                    // Try to normalize and check if it is a valid configuration after normalization
                    try {
                        val normalized = ConfigOrchestrator.orchestrate(dataString)
        ...
        try {
            ...
            val content =
                try {
                    ...
                    // Try to normalize and check if it is a valid configuration after normalization
                    try {
                        val normalized = ConfigOrchestrator.orchestrate(data)
                        Libbox.checkConfig(normalized)
                        return@withContext QRCodeParseResult.LocalProfile(name = "Imported QR Config")
                    } catch (ex: Exception) {
                        // ignore and fall through
                    }
                    return@withContext QRCodeParseResult.Error(
                ...
                val content =
                    try {
                        ...
                        try {
                            val normalized = ConfigOrchestrator.orchestrate(data)
                            Libbox.checkConfig(normalized)
                            return@withContext importJsonConfiguration(data, "Imported QR Config")
                        } catch (ex: Exception) {
                            // ignore and fall through
                        }
                        return@withContext ImportResult.Error(
    ...
    private suspend fun importProfile(content: ProfileContent): ImportResult {
        ...
        val normalizedConfig = ConfigOrchestrator.orchestrate(content.config)
        configFile.writeText(normalizedConfig)
    ...
    private suspend fun importJsonConfiguration(jsonContent: String, profileName: String): ImportResult {
        return try {
            val normalizedJsonContent = ConfigOrchestrator.orchestrate(jsonContent)
            // Validate the JSON configuration using sing-box
            try {
                // Try to check the configuration
                Libbox.checkConfig(normalizedJsonContent)
            } catch (e: Exception) {
            ...
            val configDirectory = File(context.filesDir, "configs").also { it.mkdirs() }
            val configFile = File(configDirectory, "$fileID.json")
            configFile.writeText(normalizedJsonContent)
            typedProfile.path = configFile.path
