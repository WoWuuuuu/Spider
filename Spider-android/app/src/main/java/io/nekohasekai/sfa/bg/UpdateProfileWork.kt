        ...
        override suspend fun doWork(): Result {
            ...
            for (profile in remoteProfiles) {
                ...
                try {
                    ...
                    val normalizedContent = ConfigOrchestrator.orchestrate(content)
                    Libbox.checkConfig(normalizedContent)
                    val file = File(profile.typed.path)
                    if (!file.exists() || file.readText() != normalizedContent) {
                        File(profile.typed.path).writeText(normalizedContent)
